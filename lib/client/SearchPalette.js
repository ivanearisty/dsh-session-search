...
function highlight(text, terms) {
    ...
    return parts.map((part, i) => (i % 2 === 1 ? _jsx("mark", { children: part }, i) : part));
}
...
function groupHits(hits) {
    ...
    for (const g of order)
        g.hits.sort((a, b) => a.seq - b.seq);
    return order;
}
export function SearchPalette({ useSearch, setQuery, close, pick, resurrect }) {
    ensureCss();
    ...
    useEffect(() => {
        const el = list.current?.querySelector(`[data-index="${String(cursor)}"]`);
    ...
    let index = -1;
    return (_jsx("div", { className: "ks-scrim", role: "presentation", onMouseDown: (e) => { if (e.target === e.currentTarget)
            close(); }, children: _jsxs("div", { className: "ks-card", role: "dialog", "aria-modal": "true", "aria-label": "Search messages", onKeyDown: onKeyDown, children: [_jsxs("div", { className: "ks-inputRow", children: [_jsxs("svg", { width: "16", height: "16", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": "true", children: [_jsx("circle", { cx: "11", cy: "11", r: "7" }), _jsx("path", { d: "m20 20-3.5-3.5" })] }), _jsx("input", { ref: input, className: "ks-input", type: "text", value: query, placeholder: "Search every conversation\u2026", spellCheck: false, autoComplete: "off", "aria-label": "Search query", "aria-controls": "dsh-session-search-results", onChange: (e) => { setQuery(e.currentTarget.value); } }), _jsx("span", { className: "ks-kbd", children: "esc" })] }), _jsxs("div", { className: "ks-list", id: "dsh-session-search-results", role: "listbox", ref: list, children: [query.trim().length === 0 && _jsxs("div", { className: "ks-empty", children: ["Type to search across ", sessionsIndexed, " sessions \u00B7 ", documents, " messages"] }), query.trim().length > 0 && status === 'ready' && flat.length === 0 && _jsxs("div", { className: "ks-empty", children: ["No matches for \u201C", query, "\u201D"] }), status === 'error' && _jsx("div", { className: "ks-empty", role: "alert", children: error }), resurrecting !== null && _jsx("div", { className: "ks-empty", role: "status", children: "Resurrecting \u2014 the server restarts (~15 s); the session returns to your sidebar when the GUI is back." }), groups.map(group => {
                            const tomb = archived.includes(group.sessionId);
                            return (_jsxs("div", { children: [_jsxs("div", { className: "ks-group", children: [tomb && _jsx("span", { "aria-hidden": "true", children: "\uD83E\uDEA6" }), " ", group.title, " ", _jsxs("small", { children: [group.hits.length, " hit", group.hits.length === 1 ? '' : 's'] }), tomb && _jsx("span", { className: "ks-tomb", children: "archived" }), tomb && canResurrect && _jsx("button", { type: "button", className: "ks-btn", disabled: resurrecting !== null, onClick: () => { void resurrect(group.sessionId); }, children: resurrecting === group.sessionId ? 'Rising…' : 'Resurrect' })] }), group.hits.map((hit) => {
                                        index += 1;
                                        const i = index;
                                        return (_jsxs("button", { type: "button", role: "option", className: "ks-row", "data-index": i, "data-archived": tomb, "aria-selected": i === cursor, "aria-disabled": tomb, title: tomb ? 'Archived — resurrect the session to open it' : undefined, onMouseEnter: () => { setCursor(i); }, onClick: () => { pick(hit); }, children: [_jsxs("div", { className: "ks-meta", children: [_jsx("span", { className: "ks-role", "data-role": hit.role, children: hit.role === 'user' ? 'You' : 'Assistant' }), _jsx("span", { children: when(hit.time) })] }), _jsx("div", { className: "ks-snip", children: highlight(hit.snippet, hit.terms) })] }, `${hit.sessionId}#${String(hit.seq)}`));
                                    })] }, group.sessionId));
                        })] }), _jsxs("div", { className: "ks-foot", children: [_jsxs("span", { children: [_jsx("kbd", { className: "ks-kbd", children: "\u2191\u2193" }), " navigate ", _jsx("kbd", { className: "ks-kbd", children: "\u21B5" }), " open"] }), _jsx("span", { children: status === 'searching' ? 'searching…' : `${flat.length} result${flat.length === 1 ? '' : 's'}` })] })] }) }));
}
