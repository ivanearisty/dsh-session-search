...
export class SessionIndex {
    ...
    async buildFromPersistence(persistence, live) {
        ...
        for (const header of headers)
            this.knownSessions.add(header.id);
    ...
    indexEvents(header, events) {
        ...
        const inherited = parent !== undefined && this.knownSessions.has(parent)
            ? header.seedLength ?? 0
            : 0;
        const pending = [];
    ...
    search(rawQuery, limit) {
        ...
        if (results.length === 0)
            results = this.engine.search(query, { ...SEARCH_OPTIONS, combineWith: 'OR' });
        const hits = [];
